 ...
 *
 * A `null` scope marks a global (instance-wide) invocation: nested worker→host
 * calls made under it are not confined to a single company. The host uses this
 * for inbound dispatches whose company is unknown at dispatch time and is
 * resolved by the worker (e.g. a webhook interaction routed by external
